...
public class TaskOverlayFactory implements ResourceBasedOverride {
    ...
    public static class TaskOverlay<T extends OverviewActionsView> {
        ...
        protected class OverlayUICallbacksImpl implements OverlayUICallbacks {
            ...
            public void onSelect() {
                // Enable text/content selection mode for the current task
                // This will allow users to select text or content from the app screenshot
                endLiveTileMode(() -> {
                    if (mIsAllowedByPolicy) {
                        mImageApi.startSelection(getThumbnail(),
                                getTaskSnapshotBounds(), getTaskSnapshotInsets(), mTask.key);
                    } else {
                        showBlockedByPolicyMessage();
                    }
                });
    ...
    public interface OverlayUICallbacks {
        ...
        /** User wants to select text or content from the current task. */
        void onSelect();
